# Authors and Citation

## Authors

- **Rob Hyndman**. Author, maintainer, copyright holder.
  [](https://orcid.org/0000-0002-2140-5352)

- **Torben Tvedebrink**. Contributor.

- **Posit Software, PBC**. Copyright holder.

## Citation

Source:
[`inst/CITATION`](https://github.com/robjhyndman/weird/blob/v3.0.0/inst/CITATION)

Hyndman RJ (2026). *weird: Functions and Data Sets for "That's Weird:
Anomaly Detection Using R" by Rob J Hyndman*. R package version 3.0.0,
<https://pkg.robjhyndman.com/weird/>.

    @Manual{,
      title = {{weird: Functions and Data Sets for "That's Weird: Anomaly Detection Using R" by Rob J Hyndman}},
      author = {Rob J Hyndman},
      year = {2026},
      note = {R package version 3.0.0},
      url = {https://pkg.robjhyndman.com/weird/},
    }

Hyndman RJ (2026). *That's Weird: Anomaly Detection Using R*. OTexts,
Melbourne, Australia. In preparation, <https://OTexts.com/weird/>.

    @Book{,
      title = {That's Weird: Anomaly Detection Using {R}},
      author = {Rob J Hyndman},
      year = {2026},
      note = {In preparation},
      publisher = {OTexts},
      address = {Melbourne, Australia},
      url = {https://OTexts.com/weird/},
    }
