...
#' @param log_scores Numerical vector of log scores
...
#' tibble(
#'   y = c(5, rnorm(49)),
#'   logscores = log_scores(y),
#'   loglooscores = log_scores(y, loo = TRUE),
#'   lookout = lookout(logscores, loglooscores)
#' )
#' # Bivariate data
#' tibble(
#'   x = rnorm(50),
...
lookout <- function(log_scores, log_loo_scores = log_scores, threshold_probability = 0.95) {
  threshold <- stats::quantile(log_scores, prob = threshold_probability, type = 8)
  gpd <- evd::fpot(log_scores, threshold = threshold, std.err = FALSE)$estimate
